...
const AddAccessForm: React.FC<AddAccessFormProps> = ({ open, onClose, entityId, onSuccess, onFail, userRights }) => {
  ...
  const [startDate, setStartDate] = useState<MaterialUiPickersDate | null>(moment()) // pas de date antérieure à today autorisée
  ...
  useEffect(() => {
    const _getAssignableRoles = async () => {
      try {
        ...
        const assignableRolesResp = await getAssignableRoles(careSite?.care_site_id)
  ...
  const onSubmit = async () => {
    try {
      const stringStartDate =
        moment(startDate).isValid() && !moment(startDate).isSame(new Date(), 'day') ? moment(startDate).format() : null

      const stringEndDate = moment(endDate).isValid() ? moment(endDate).format() : null
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      <DialogTitle className={classes.title}>Créer un nouvel accès :</DialogTitle>
      ...
      <DialogActions>
        ...
        <Button disabled={!careSite || !role || dateError} onClick={onSubmit} color="primary">
          Valider
        </Button>
      </DialogActions>
