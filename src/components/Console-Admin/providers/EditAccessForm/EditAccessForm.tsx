...
const EditAccessForm: React.FC<EditAccessFormProps> = ({ open, onClose, access, onSuccess, onFail }) => {
  const classes = useStyles()

  const [startDate, setStartDate] = useState<MaterialUiPickersDate | null>(
    moment(new Date(access?.actual_start_datetime as string), 'YYYY-MM-DD[T]HH:mm:ss Z')
  )
  const [endDate, setEndDate] = useState<MaterialUiPickersDate | null>(
    moment(new Date(access?.actual_end_datetime as string), 'YYYY-MM-DD[T]HH:mm:ss Z')
  )
  ...
  useEffect(() => {
    setStartDate(moment(new Date(access?.actual_start_datetime as string), 'YYYY-MM-DD[T]HH:mm:ss Z'))
    setEndDate(moment(new Date(access?.actual_end_datetime as string), 'YYYY-MM-DD[T]HH:mm:ss Z'))
  }, [access])
  ...
  const onSubmit = async () => {
    try {
      ...
      const stringEndDate = moment(endDate).isValid() ? moment(endDate).format() : access?.actual_end_datetime ?? null
      ...
      const submitEditAccessResp = await submitEditAccess(editAccessData, access?.care_site_history_id)
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      <DialogTitle className={classes.title}>Éditer l'accès à {access?.care_site.care_site_name}:</DialogTitle>
          ...
          {!startDate?.isBefore() && (
            <Grid container justify="space-between" alignItems="center" className={classes.filter}>
              ...
              <KeyboardDatePicker
                ...
                format="DD/MM/YYYY"
                onChange={(date: MaterialUiPickersDate) => setStartDate(date ?? null)}
                value={startDate}
              />
      ...
      <DialogActions>
        ...
        <Button disabled={dateError || (isStartDatePast && isEndDatePast)} onClick={onSubmit} color="primary">
          Valider
        </Button>
      </DialogActions>
    </Dialog>
