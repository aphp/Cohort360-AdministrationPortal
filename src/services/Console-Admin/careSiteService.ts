...
const loadingItem: ScopeTreeRow = { care_site_id: 'loading', name: 'loading', children: [] }
...
export const getScopeCareSites = async (getCareSites: any) => {
    ...
    for (const careSite of careSitesResult){
        ...
        scopeRow.children = careSite.children?.length > 0 ? parseChildren(careSite.children) : await getCareSitesChildren(careSite as ScopeTreeRow)
